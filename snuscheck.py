...
def send_welcome(message: Message):
    ...
    bot.reply_to(
        message, "Gửi file domains.txt để bắt đầu tra cứu email trên Snusbase."
    )
...
def handle_document(message: Message):
    ...
    try:
            ...
            for i in range(0, len(domains), batch_size):
                ...
                try:
                    ...
                    if (
                        ...
                        bot.reply_to(
                            message,
                            f"🚨 DỪNG XỬ LÝ: Tất cả API keys đều bị lỗi hoặc rate limit tại batch {batch_index}",
                        )
                        should_stop = True
                        break
                        ...
                        bot.reply_to(
                            message,
                            f"⚠️ Lỗi batch {batch_index}: Đã thử tất cả API key nhưng không thành công. Vui lòng kiểm tra lại các key trong config.json",
                        )
                        should_stop = False
                        break
                        ...
                        bot.reply_to(
                            message,
                            f"⚠️ Lỗi batch {batch_index}: API key bị lỗi hoặc rate limit, đã tự động chuyển sang key khác",
                        )
                    else:
        ...
        with open(progress_file, "w", encoding="utf-8") as f_progress:
            ...
            if should_stop:
                f_progress.write(
                    f"Trạng thái: DỪNG XỬ LÝ do tất cả API keys đều bị lỗi\n"
                )
                if failed_batches:
                    last_failed = failed_batches[-1]
                    f_progress.write(
                        f"Điểm dừng: Batch {last_failed['batch_index']} (Index {last_failed['start_index']}-{last_failed['end_index']-1})\n"
                    )
            else:
        ...
        with open(progress_file, "rb") as f:
            if should_stop:
                bot.send_document(
                    message.chat.id,
                    f,
                    caption="📊 Báo cáo tiến độ (ĐÃ DỪNG)",
                    reply_to_message_id=message.message_id,
                )
            else:
                bot.send_document(
                    message.chat.id,
                    f,
                    caption="📊 Báo cáo tiến độ (HOÀN THÀNH)",
                    reply_to_message_id=message.message_id,
                )
        ...
        if should_stop:
            logging.info(
                f"Sent result files to user {user_id}: DỪNG XỬ LÝ - {len(written_emails)} emails, {len(processed_domains)} domains processed"
            )
        else:
            logging.info(
                f"Sent result files to user {user_id}: HOÀN THÀNH - {len(written_emails)} emails, {len(processed_domains)} domains processed"
            )
